...
var TaskProvider = function (baseUrl) {
   ...
   self.detail = (taskid) => {
    ...
    self.exportTo = (provider, categoryId, count = 100) => {
        return new Promise ((resolve, reject) => {
            ...
            switch (provider) {
                case 'es' :
                        provider = require('../import/es.js');
                    break;
                default : 
                    reject(new Error('UNKNOWN_IMPORT_PROVIDER'));         
            }

            //// Need to grab every task with update & comments
            self.list(categoryId)   
                .then((list) => {
